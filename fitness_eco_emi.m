    ...
    for i = 1 : popSize
        ...
        for j =1 : pPartikel
            if arrIndiv(i,j) == 0
                ...
                totalEcoEmi =  totalEcoEmi + ( ((arrCostEco(j, 1) + (arrCostEco(j,2) * arrIndiv(i,j)) + (arrCostEco(j, 3) * power(arrIndiv(i,j),2))) /1000) + (arrCostEmi(j, 1) + (arrCostEmi(j,2) * arrIndiv(i,j)) + (arrCostEmi(j, 3) * power(arrIndiv(i,j),2))));  
            end
        end

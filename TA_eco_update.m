...
%      arrBatasV(1, i) = arrBatas(i, 1);
%      arrBatasV(2, i) = (arrBatas(i, 2) - arrBatas(i, 1));
end
...
% %             k = randi([1 , pPartikel]);
% %             disp(k);
%             if (arrIndiv(j,shuffle(k)) >= arrBatas(shuffle(k),1))  
%                  if (arrIndiv(j,shuffle(k)) + arrHitung(j)) >= arrBatas(shuffle(k),1)  && (arrIndiv(j,shuffle(k)) + arrHitung(j)) <= arrBatas(shuffle(k),2)
%                      if arrHitung(j) < 0
%                      arrIndiv(j,shuffle(k)) = arrIndiv(j,shuffle(k)) + arrHitung(j);
%                      shuffle2 = test_shuffle();
%                      tempHitung = arrHitung(j) * -1;
%                          for x = 1 :pPartikel
%                             if arrIndiv(j, shuffle2(x)) + tempHitung >= arrBatas(shuffle2(x),1) && arrIndiv(j, shuffle2(x)) + tempHitung <= arrBatas(shuffle2(x),2) && (shuffle2(x) ~= shuffle(k))
%                               arrIndiv(j, shuffle2(x)) =   arrIndiv(j, shuffle2(x)) + tempHitung ;
%                               break;
%                             end
%                          end
%                          break;

function fitness = fitness_emi(arrIndiv, popSize, pPartikel)
    ...
    for i = 1 : popSize
        ...
        for j =1 : pPartikel
            if arrIndiv(i,j) == 0
                ...
                totalEmi =  totalEmi + (arrCost(j, 1) + (arrCost(j,2) * arrIndiv(i,j)) + (arrCost(j, 3) * power(arrIndiv(i,j),2)));  
            end
        end

        fitness(i, 1) = totalEmi;
    end
end

...
fitness = fitness_emi(arrIndiv, popSize, pPartikel);
% fitness
...
%     fitnessGbest
    ...
    fprintf("Dengan Emisi adalah :  %s Ton/Jam\n", formatted_average);
...
for k = 1 : pPartikel
    if arrGbest(1, k) ~= 0
        ...
        emi = num2str(hitungEmi(k, arrGbest(1,k)), '%.2f, ');
        emi = insertCommas(emi);
        fprintf("Generator ke - %d : dengan Beban %.2f => Emisi : %s Ton/Jam\n", k, arrGbest(1,k), emi);
    end
end

...
%     fitnessGbest
    ...
    totalEmi = 0;
    for k = 1 : pPartikel
        nEco = hitungEco(k, arrGbest(1,k));
        totalEco = totalEco + nEco;
        
        nEmi = hitungEmi(k, arrGbest(1,k));
        totalEmi = totalEmi + nEmi;    
    end   
    eco = num2str(totalEco, '%.2f, ');
    eco = insertCommas(eco);
    
    emi = num2str(totalEmi, '%.2f, ');
    emi = insertCommas(emi);
    
    fprintf("Dengan Total Biaya adalah : Rp %s\n", eco);
    fprintf("Dengan Total Emisi adalah : %s Ton/Jam\n", emi);
...
for k = 1 : pPartikel
    if arrGbest(1, k) ~= 0
        ...
        emi = num2str(nEmi, '%.2f, ');
        emi = insertCommas(emi);
        fprintf("Generator ke - %d : dengan Beban %.2f => Biaya : Rp %s => Emisi : %s Ton/Jam \n", k, arrGbest(1,k), eco, emi);
    end
end

fprintf("Total Beban yang dicover adalah : %.2f\n", totalBeban);

formatted_eco = num2str(totalEco, '%.2f, ');
formatted_eco = insertCommas(formatted_eco);
fprintf("Dengan Total Biaya adalah : Rp %s \n", formatted_eco);

formatted_numbers = num2str(totalEmi, '%.2f, ');
